...
<section class="relative h-full w-full">
	...
	<div class="relative flex-col gap-4 p-5">
		<h4 class="text-lg font-bold lg:text-2xl">Book of the day</h4>
		<div class="flex items-center justify-between">
			<div class=" flex-col gap-4 w-2/3">
				<h3 class="line-clamp-2 text-2xl font-bold">{book?.volumeInfo?.title}</h3>
				<p class="line-clamp-3 text-sm pt-5">{book?.volumeInfo?.description}</p>
			</div>
			<enhanced:img
				src={book?.volumeInfo?.imageLinks?.thumbnail || '/no-cover.png'}
				class=" rounded-lg w-1/3 h-48 object-contain"
			/>
		</div>
